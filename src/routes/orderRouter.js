...
orderRouter.post(
  ...
  authRouter.authenticateToken,
  asyncHandler(async (req, res) => {
    const startTime = Date.now(); // Fixed: use Date.now() instead of new Date().now
    ...
    try {
      ...
      if (r.ok) {
        metrics.incrementPizzasSoldSuccess();
        // Calculate and track revenue
        const orderTotal = order.items.reduce((total, item) => total + item.price, 0);
        metrics.incrementPizzasRevenue(orderTotal);
        res.send({ order, jwt: j.jwt, reportUrl: j.reportUrl });
      } else {
        metrics.incrementPizzasSoldFailed();
        res
          .status(500)
